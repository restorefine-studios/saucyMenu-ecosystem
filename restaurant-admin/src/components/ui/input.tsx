	...
	({ className, type, ...props }, ref) => {
		return (
			<input
				type={type}
				className={cn(
					"flex h-10 w-full rounded-md border border-input bg-background px-2 py-2 text-base ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-foreground placeholder:text-muted-foreground disabled:cursor-not-allowed disabled:opacity-50 md:text-sm",
					className,
				)}
				ref={ref}
				{...props}
	...
	({ label, errors, className, ...props }, ref) => {
		return (
			<div className="grid w-full gap-1">
				<label className={"w-fit text-sm font-semibold text-gray-600"}>
					{label}
				</label>
				<Input
					ref={ref}
					{...props}
					className={cn("py-7 bg-[#f8f8f8] indent-1.5 w-full", className)}
				/>
				{errors && (
