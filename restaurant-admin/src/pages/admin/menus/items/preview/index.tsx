...
const PreviewItem = ({ selectedDish }: Props) => {
  ...
  const handleEdit = () => {
    navigate(
      `/admin/menus/${selectedDish.sectionId}/items/${selectedDish.id}/edit/${selectedDish.id}`
    );
  };
    ...
    <div className="flex flex-col w-full max-h-[80vh] overflow-y-auto p-0">
      ...
      <div className="py-3 flex flex-col gap-y-5 ">
        <div className="flex justify-between items-start">
          <div className="">
            ...
            <h3 className="hidden text-sm font-medium text-gray-500 mt-0">
              {t("admin.menus.items.preview.description")}
            </h3>
        ...
        {selectedDish?.tags?.length > 0 && (
          <div className="flex flex-col items-start gap-0">
            <h4 className="font-medium text-sm text-gray-600 mb-0">
              {t("admin.menus.items.preview.diet")}
            </h4>

            <div className="flex flex-wrap gap-1">
              {selectedDish?.tags?.map((tag) => (
                <div
                  className="bg-gray-200 w-fit h-auto p-1 px-3 text-black text-xs rounded-sm capitalize"
                  key={tag.name}
                >
                  {tag.name}
                </div>
        ...
        {selectedDish?.allergens?.length > 0 && (
          <div className="flex flex-col items-start gap-0">
            <h4 className="font-medium text-sm text-gray-600 mb-0">
              {t("admin.menus.items.preview.allergens")}
            </h4>
            <div>
              <div className="flex flex-wrap gap-1">
                {selectedDish?.allergens?.map((tag) => (
                  <div
                    className="bg-gray-200 w-fit h-auto p-1 px-3 text-black text-xs rounded-sm capitalize"
                    key={tag.name}
                  >
        ...
        {selectedDish.ingredients &&
          selectedDish.ingredients.filter((i) => i.trim() !== "").length >
            ...
            <div className="flex flex-col items-start gap-0">
              <h4 className="font-medium text-sm text-gray-600 mb-0">
                {t("admin.menus.items.preview.ingredients")}
              </h4>
              <div className="flex flex-wrap gap-1">
        ...
        <div className="flex justify-end ">
          <Button onClick={handleEdit} className="hover:cursor-pointer">
            {t("admin.menus.items.preview.editDish")}
          </Button>
        </div>
      </div>
    </div>
